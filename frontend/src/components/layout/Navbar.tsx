            ...
            {NAV_LINKS.map(link => (
              link.highlight ? (
                <Link
                  key={link.href}
                  href={link.href}
                  className="font-sans text-xs tracking-[0.14em] uppercase transition-all duration-200 relative group"
                  style={{ color: '#C6A9A3' }}
                  onMouseEnter={e => (e.currentTarget.style.color = '#A08070')}
                  onMouseLeave={e => (e.currentTarget.style.color = '#C6A9A3')}
                >
                  ✦ {link.label}
            ...
            <nav className="flex flex-col gap-6">
              {NAV_LINKS.map(link => (
                <Link key={link.href} href={link.href}
                  ...
                  style={{ color: link.highlight ? '#C6A9A3' : '#3E3A39' }}
                >
