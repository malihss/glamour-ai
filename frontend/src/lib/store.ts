...
// ── Ratings Store (static, localStorage) ──────────────────────────────────
interface ProductRatingEntry {
  total: number
  count: number
...
interface RatingsState {
  ratings: Record<string, ProductRatingEntry>
  rate: (productId: string, newRating: number) => void
  getAvg: (productId: string) => number | null
...
export const useRatingsStore = create<RatingsState>()(
  persist(
    (set, get) => ({
      ...
      rate: (productId, newRating) => {
        const { ratings } = get()
        const existing = ratings[productId]
        if (existing) {
          const prevUser = existing.userRating ?? 0
          const newTotal = existing.total - prevUser + newRating
          set({
            ratings: {
              ...ratings,
              [productId]: {
                total: newTotal,
                count: prevUser === 0 ? existing.count + 1 : existing.count,
          ...
          set({
            ratings: {
              ...ratings,
              [productId]: { total: newRating, count: 1, userRating: newRating },
            },
          })
        }
      },
      ...
      getAvg: (productId) => {
        const entry = get().ratings[productId]
        if (!entry || entry.count === 0) return null
        return Math.round((entry.total / entry.count) * 10) / 10
      },

      getCount: (productId) => get().ratings[productId]?.count ?? 0,

      getUserRating: (productId) => get().ratings[productId]?.userRating ?? null,
    }),
    {
      name: 'glamour-ratings',
      storage: createJSONStorage(() => localStorage),
    }
  )
)
